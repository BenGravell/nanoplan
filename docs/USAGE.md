...
| **planner** dropdown | Selects which of the five planners drives the ego vehicle. If this (scenario, planner) combo hasn't been simulated yet, a "Simulate" button (then a progress bar while it runs) appears in place of the metrics table below — simulating doesn't block the window, so an expensive planner like PI²-DDP or RRT* never freezes the UI. Re-selecting a combo already simulated is instant. |
