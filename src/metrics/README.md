...
```
metrics/
├── mod.rs               Metrics struct, METRICS spec table, evaluate() orchestrator
├── aggregation.rs       min/average aggregation and composite formula
...
just once per scenario. `Metrics::per_tick[tick]` is a 9-element array (one
...
`aggregation::composite()` folds over the table: the product of every
`Multiplier` metric times the weighted average of the `Weighted` ones. This
reproduces nuPlan's structure — the four multiplier metrics (collisions,
drivable area, driving direction, making progress) gate the score entirely,
and the five weighted metrics (TTC, progress, speed limit, comfort, lane
keeping at 5/5/4/2/2) blend smoothly. This same
formula is applied twice per rollout: once per tick (`score_per_tick`, for
the `@t` column and future use) and once to the aggregates
(`Metrics::score`, the number everything else — the viewer's "closed-loop
score" and the batch runner's CSV `score` column — actually reports).

## The nine metrics

| # | Metric | Module | Rule | What it checks | Key thresholds |
|---|---|---|---|---|---|
| 0 | No at-fault collisions | `collisions` | min | Ego's rendered rectangular footprint does not overlap any actor footprint. | Footprints come from `geometry::{EGO_FOOTPRINT, CAR_FOOTPRINT}` |
| 1 | Drivable area | `drivable_area` | min | Ego's full footprint stays inside the road's barrier/lane geometry, including finite segment extent. | Scenario `map.road_half_width`; no default |
...
| 4 | TTC within bound | `ttc` | min | Constant-velocity projections of ego and every actor, sampled every `0.1` s out to `3.0` s, never overlap as rendered footprints. | `LEAST_MIN_TTC_S = 0.95` |
...
and lateral-offset series (via one `scenarios::Path` projection pass), the
ego footprint's drivable-area compliance against road geometry, and the
`comfort::Kinematics` (forward-differenced accel/yaw-rate)
...
`aggregation::composite()` call happen once, after the tickwise loop.
