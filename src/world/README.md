...
single-track traffic with kinematic gap control, passes only actors whose
reachable interval can overlap the ego's, calls the selected planner, and
applies its first control. Cars that fall far behind are recycled ahead.
